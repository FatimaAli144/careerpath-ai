...
function buildSystemPrompt(profile, jobs) {
  let profileSection =
    "No profile yet — encourage the student to complete their profile.";
  ...
  if (profile && (profile.fullName || profile.university)) {
    ...
    if (profile.targetRoles) {
      lines.push(`Target roles: ${profile.targetRoles}`);
    }

    if (profile.workMode) {
      lines.push(`Work mode: ${profile.workMode}`);
    }

    if (profile.salaryMin) {
      lines.push(`Min salary: ₨ ${profile.salaryMin}k/month`);
    }
...
Include 2-4 jobs max.
Omit if not recommending jobs.`;
}
